...
(async () => {
  ...
  if (url.includes('skip_api_login')) {
    ...
    await page.type('input[name="email"]', 'alexram80@me.com', { delay: 15 });
    await page.type('input[name="pass"]', 'Ironfloor88LE@N', { delay: 15 });
    await sleep(500);
  ...
  if (finalUrl.includes('two_step') || finalUrl.includes('2fa')) {
    console.log('\n⚠️ 2FA - Revisa tu telefono y aprueba');
    console.log('Esperando hasta 2 minutos...');
    await page.waitForNavigation({ timeout: 120000 }).catch(() => {});
    const afterUrl = page.url();
    console.log('URL post-2FA:', afterUrl.substring(0, 200));
  ...
  await page.screenshot({ path: '/tmp/oauth4.png' });
